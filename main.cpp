...
#define hdte_VERSION "0.0.1"
#define hdte_TAB_STOP 8
#define hdte_QUIT_TIMES 3
...
int editorRowCxToRx(erow *row, int cx) {
    ...
    for (j = 0; j < cx; j++) {
        if (row->chars[j] == '\t')
            rx += (hdte_TAB_STOP - 1) - (rx % hdte_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t')
            cur_rx += (hdte_TAB_STOP - 1) - (cur_rx % hdte_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = static_cast<char *>(malloc(row->size + tabs * (hdte_TAB_STOP - 1) + 1));

    int idx = 0;
    for (j = 0; j < row->size; j++) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % hdte_TAB_STOP != 0) row->render[idx++] = ' ';
        } else {
...
void abAppend(struct abuf *ab, const char *s, int len) {
    char *newd = static_cast<char *>(realloc(ab->b, ab->len + len));
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0 && y == E.screenrows / 3) {
                ...
                int welcomelen = snprintf(welcome, sizeof(welcome),
                                          "hdte editor -- version %s", hdte_VERSION);
...
void editorProcessKeypress() {
    static int quit_times = hdte_QUIT_TIMES;
